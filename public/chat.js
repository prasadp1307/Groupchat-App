...
async function getmsgs() {
    ...
    try {
        ...
        if (!localStorage.getItem(`savedGroup${activeGroupId}`)) {
            ...
            const AllTextMessages = await axios.get(`http://localhost:4000/chat/getText/${activeGroupId}`, { headers: { Authorization: token } });
        ...
        intervalId = setInterval(async () => {
            try {
                ...
                const newMsgs = await axios.post(`http://localhost:4000/chat/realTime/${localStorage.getItem('groupId')}`,
                    {
...
async function sendMessage(e) {
    ...
    try {
        ...
        if (messageText.value || uploadFile.value) {
            ...
            const sendMessageRes = await axios.post('http://localhost:4000/chat/sendMessage', formData, {
                headers: {
